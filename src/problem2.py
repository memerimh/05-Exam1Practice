...
def problem2b(rect, n, delta, win):
    ...
    """
    rect.attach_to(win)
    upleft = rect.get_upper_left_corner()
    downright = rect.get_lower_right_corner()
    for k in range(n):
        dist = ((k + 1) * delta)
        nurectangle = rg.Rectangle(rg.Point(upleft.x - dist, upleft.y - dist),
                                   rg.Point(downright.x + dist, downright.y + dist))
        nurectangle.attach_to(win)
    win.render()
